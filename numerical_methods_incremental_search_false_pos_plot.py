...
def main_loop():
    ...
    while not terminate:
        ...
        results, xx, yy = incremental(fun, (xl, xu), N) # search for zero crossings, results is list of lists [xl, xu]
        roots = []
        if len(results)>0:
        ...
        plt.plot(xx, yy, label = fun)
        for root in roots:
            plt.scatter(root, 0, label = f"root: xr={root:.8f}", s = 80)
        plt.grid()
        plt.legend()
        plt.title(f"Plot and roots of {fun}=0")
        plt.show()
